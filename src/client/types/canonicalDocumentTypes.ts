 ...
 */
export function isDocumentApproved(doc: CanonicalDocument): boolean {
  return doc.enrichmentMetadata?.accepted === true;
}

/**
 * Type guard to check if a document is rejected
 */
export function isDocumentRejected(doc: CanonicalDocument): boolean {
  return doc.enrichmentMetadata?.accepted === false;
}

/**
 * Type guard to check if a document is pending review
 */
export function isDocumentPending(doc: CanonicalDocument): boolean {
 ...
 */
export type DocumentOperationHandler = (
  document: CanonicalDocument,
