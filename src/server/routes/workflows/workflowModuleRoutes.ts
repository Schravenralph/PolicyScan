 ...
 *
 * Handles endpoints for workflow module discovery and metadata:
 * - GET /api/workflows/modules - List all available modules
 * - GET /api/workflows/modules/:id - Get specific module by ID
 * - GET /api/workflows/modules/category/:category - Get modules by category
 */
...
/**
 * Create router for workflow module endpoints
 *
 * @returns Express router with module routes
 */
export function createWorkflowModuleRouter(): Router {
    const router = Router();

    // GET /api/workflows/modules
    // Get all available workflow modules
    router.get('/workflows/modules', asyncHandler(async (_req: Request, res: Response) => {
    ...
    router.get('/workflows/modules/:id', validate(workflowSchemas.getModule), asyncHandler(async (req: Request, res: Response) => {
        const { moduleRegistry } = await import('../../services/workflow/WorkflowModuleRegistry.js');
        const registry = moduleRegistry;
        const module = registry.get(req.params.id);

        if (!module) {
            throw new NotFoundError('Module', req.params.id);
        }
