...
async function backfillSource() {
    ...
    const recordsWithoutSourceQuery: Filter<CDXIndexRecord> = {
        $or: [
            { source: { $exists: false } },
            { source: undefined }
        ]
    } as Filter<CDXIndexRecord>;
    const recordsWithoutSource = await collection.countDocuments(recordsWithoutSourceQuery);
    const recordsWithSource = await collection.countDocuments({
        source: { $exists: true, $ne: undefined }
    } as unknown as Filter<CDXIndexRecord>);
    ...
    const updateFilter: Filter<CDXIndexRecord> = {
        $or: [
            { source: { $exists: false } },
            { source: undefined }
        ]
    ...
    const verifyFilter: Filter<CDXIndexRecord> = {
        $or: [
            { source: { $exists: false } },
            { source: undefined }
        ]
    };
    const remainingWithoutSource = await collection.countDocuments(verifyFilter);
