...
import { HybridScorer, HybridScoringInput } from '../graphrag/HybridScorer.js';
...
export interface RankingFactors {
    ...
    rerankerScore?: number;    // Optional: Score from LLM re-ranker
    hybridScore?: number;      // Optional: Combined score before re-ranking
}
...
export class RankingService {
    ...
    constructor(
        ...
        // Initialize HybridScorer if vector service is provided and feature flag is enabled
        const featureFlagsService = getFeatureFlagsService();
        if (this.vectorService && featureFlagsService.isHybridScoringEnabled()) {
            this.hybridScorer = new HybridScorer(this.vectorService, {}, this.knowledgeGraph);
        }
     ...
     * Uses hybrid scoring when enabled, otherwise falls back to basic scoring
    ...
    async calculateRankScore(
        ...
        // Check if hybrid scoring is enabled and we have the required dependencies
        const featureFlagsService = getFeatureFlagsService();
        if (featureFlagsService.isHybridScoringEnabled() && entity && this.vectorService) {
            return await this.calculateRankScoreWithHybridScoring(vectorScore, entity, metadata, query);
        } else {
            return await this.calculateRankScoreBasic(vectorScore, entity, metadata, query);
        }
    }

    /**
     * Calculate ranking score using hybrid scoring (KG confidence + vector similarity)
     */
    private async calculateRankScoreWithHybridScoring(
        vectorScore: number,
        entity: BaseEntity,
        metadata?: Record<string, unknown>,
        query?: { onderwerp?: string; thema?: string; overheidslaag?: string }
    ): Promise<RankingFactors> {
        // Initialize hybrid scorer if needed
        if (!this.hybridScorer) {
            this.hybridScorer = new HybridScorer(this.vectorService!, {}, this.knowledgeGraph);
        }

        // Prepare hybrid scoring input
        const queryText = query?.onderwerp || '';
        const scoringInput: HybridScoringInput = {
            entity,
            entityMetadata: {
                sourceUrls: entity.metadata?.sourceUrls as string[] | undefined,
                extractionTimestamp: entity.metadata?.extractionTimestamp
                    ? (typeof entity.metadata.extractionTimestamp === 'string'
                        ? new Date(entity.metadata.extractionTimestamp)
                        : entity.metadata.extractionTimestamp as Date)
                    : undefined,
            },
        ...
        const hybridScore = await this.hybridScorer.calculateScore(scoringInput);

        // Calculate recency score
        const recencyScore = metadata?.date || metadata?.publicatiedatum
            ? this.calculateRecencyScore((metadata.date || metadata.publicatiedatum) as string)
            : 0.5;
        ...
        // Combine hybrid score with recency and metadata
        // Hybrid score already combines KG and vector, so we use it as the base
        // Then add recency and metadata with their weights
        const finalScore =
            hybridScore.finalScore * (this.config.vectorWeight + this.config.graphWeight) +
            recencyScore * this.config.recencyWeight +
            metadataScore * this.config.metadataWeight;

        return {
            vectorScore: hybridScore.vectorScore,
            graphRelevance: hybridScore.kgScore,
            recencyScore,
            metadataScore,
            finalScore
        };
    ...
    /**
     * Calculate ranking score using basic scoring (original implementation)
     */
    private async calculateRankScoreBasic(
        vectorScore: number,
        entity?: BaseEntity,
        metadata?: Record<string, unknown>,
        query?: { onderwerp?: string; thema?: string; overheidslaag?: string }
    ...
    private calculateMetadataScore(
        ...
        if (query?.thema && metadata.themes && Array.isArray(metadata.themes)) {
            const themeMatch = this.matchThemes(metadata.themes, query.thema);
            if (themeMatch > 0) {
        ...
        if (query?.overheidslaag && metadata.issuingAuthority) {
            const issuingAuthority = typeof metadata.issuingAuthority === 'string' ? metadata.issuingAuthority : String(metadata.issuingAuthority);
            if (this.matchAuthority(issuingAuthority, query.overheidslaag)) {
        ...
        const publicatiedatum = metadata.publicatiedatum;
        if (publicatiedatum && confidence > 0.7) {
    ...
    async calculateGraphRelevance(entity: BaseEntity): Promise<number> {
        // Use optimized method to efficiently get neighbor counts in a single query
        ...
        const outgoingDefinedIn = counts.outgoing.byType[RelationType.DEFINED_IN] || 0;
        const outgoingAppliesTo = counts.outgoing.byType[RelationType.APPLIES_TO] || 0;
        relationshipScore += (outgoingDefinedIn + outgoingAppliesTo) * 0.1;

        // Check incoming relationships
        const incomingDefinedIn = counts.incoming.byType[RelationType.DEFINED_IN] || 0;
        const incomingAppliesTo = counts.incoming.byType[RelationType.APPLIES_TO] || 0;
        relationshipScore += (incomingDefinedIn + incomingAppliesTo) * 0.1;
