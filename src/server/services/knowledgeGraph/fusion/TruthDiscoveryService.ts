...
import { ConflictResolver, Conflict, ResolutionResult, ResolutionStrategy } from '../../knowledge-graph/fusion/ConflictResolver.js';
...
export class TruthDiscoveryService {
    ...
    private extractSourceInfo(entity: BaseEntity): SourceInfo {
        const url = (entity as any).url || entity.metadata?.sourceUrl || entity.metadata?.url;
        const timestamp = (entity as any).date || entity.metadata?.createdAt || entity.metadata?.timestamp;
    ...
    private async applyResolution(conflict: Conflict, resolution: ResolutionResult): Promise<void> {
        ...
        if (conflict.property.startsWith('metadata.')) {
            ...
            (entity as any)[conflict.property] = resolution.resolvedValue;
        }
    ...
    private async saveResolution(conflict: Conflict, resolution: ResolutionResult): Promise<void> {
        try {
            await ConflictResolution.create({
                ...
                conflictValues: conflict.values.map((v: { value: unknown; source: SourceInfo; reliabilityScore: ReliabilityScore }) => ({
                    value: v.value,
