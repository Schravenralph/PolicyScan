...
import { CanonicalDocument } from '../../../contracts/types.js';
import { KnowledgeGraphService } from '../core/KnowledgeGraph.js';
import { KnowledgeGraphServiceInterface } from '../core/KnowledgeGraphInterface.js';
import { validateGraphDBBackend } from '../utils/architectureValidation.js';
import { getNeo4jDriver } from '../../../config/neo4j.js';
import { getKnowledgeGraphService } from '../core/KnowledgeGraph.js';
import { GraphDBKnowledgeGraphService } from '../core/GraphDBKnowledgeGraphService.js';
...
export class ChangeDetectionService {
    ...
    if (kgService) {
      ...
      if (backend === 'graphdb') {
        try {
          ...
          logger.error({ error }, '[ChangeDetectionService] GraphDB not available. GraphDB is required.');
          throw new Error('GraphDB is required for Knowledge Graph. Ensure GraphDB is connected.');
        }
      } else {
        throw new Error('GraphDB is required for Knowledge Graph. Set KG_BACKEND=graphdb.');
      }
    }
  }
  ...
  async detectDocumentChanges(
    ...
    preFetchedCanonicalDoc?: CanonicalDocument | null
  ): Promise<ChangeSet> {
    if (!this.isEnabled()) {
    ...
    if (preFetchedCanonicalDoc !== undefined) {
      canonicalDoc = preFetchedCanonicalDoc;
    } else {
      ...
      canonicalDoc = await documentService.findByUrl(document.url);
    }
    ...
    const changeSet: ChangeSet = {
      ...
      deletedDocuments: [],
      totalChanges: 0,
      detectionMethod: detectionMethods.length > 1 ? 'combined' : (detectionMethods[0] as 'content_hash' | 'timestamp' | 'metadata_diff' | 'entity_level_diff' | 'combined'),
      processingTimeMs: 0
    };
  ...
  async detectBatchChanges(
    ...
    for (let i = 0; i < documents.length; i += batchSize) {
      ...
      // Optimization: Fetch all canonical documents for the batch
      let canonicalDocs: CanonicalDocument[] = [];
      let batchFetchSuccessful = false;
      try {
        const { getCanonicalDocumentService } = await import('../../../services/canonical/CanonicalDocumentService.js');
        const documentService = getCanonicalDocumentService();
        const urls = batch.map(d => d.url);
        canonicalDocs = await documentService.findByUrls(urls);
        batchFetchSuccessful = true;
      } catch (error) {
        logger.warn({ error }, '[ChangeDetection] Failed to batch fetch canonical documents, falling back to individual fetch');
      }

      for (const document of batch) {
        try {
          // Find matching canonical doc
          // findByUrl matches: canonicalUrl OR sourceMetadata.legacyUrl OR sourceMetadata.url
          let matchingDoc: CanonicalDocument | null | undefined = undefined;

          if (batchFetchSuccessful) {
            matchingDoc = canonicalDocs.find(d =>
              d.canonicalUrl === document.url ||
              (d.sourceMetadata?.legacyUrl as string) === document.url ||
              (d.sourceMetadata?.url as string) === document.url
            ) || null;
          }

          const changeSet = await this.detectDocumentChanges(document, options, matchingDoc);
          ...
          logger.error({ error, url: document.url }, `[ChangeDetection] Error detecting changes for ${document.url}`);
        }
      }
    }
