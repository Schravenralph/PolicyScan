import { BaseEntity, EntityType, PolicyDocument } from '../../domain/ontology.js';
import { LocalEmbeddingProvider } from '../query/VectorService.js';
import { FeatureFlag, KGFeatureFlag } from '../../models/FeatureFlag.js';
import { MaximumWeightMatchingService } from './resolution/MaximumWeightMatchingService.js';
...
export class DeduplicationService {
    ...
    constructor(
        ...
        private findNodesByNormalizedName?: (type: BaseEntity['type'], normalizedName: string) => Promise<BaseEntity[]>
    ) {
        ...
        if (enableSemanticMatching) {
            try {
                ...
                logger.warn({ error }, 'Failed to initialize embedding provider, semantic matching disabled');
        ...
        if (FeatureFlag.isEnabled(KGFeatureFlag.KG_MAX_WEIGHT_MATCHING_ENABLED, false)) {
            try {
                ...
                logger.warn({ error }, 'Failed to initialize maximum weight matching service');
    ...
    private async getEntityEmbedding(entity: BaseEntity): Promise<number[] | null> {
        ...
        try {
            ...
            logger.warn({ error, entityId: entity.id }, 'Failed to generate embedding for entity');
            return null;
        }
    }
    ...
    async findDuplicates(entity: BaseEntity): Promise<DuplicateMatch[]> {
        ...
        const _entityTypeConfig = this.config.perEntityTypeRules?.[entity.type];
    ...
    private async findByNormalizedName(entity: BaseEntity): Promise<DuplicateMatch[]> {
        ...
        if (this.findNodesByNormalizedName) {
            // Use optimized query if available
            candidates = await this.findNodesByNormalizedName(entity.type, normalizedEntityName);
        } else {
            // Fallback to fetching all nodes of type
            candidates = await this.getNodesByType(entity.type);
        }
    ...
    private async findByMetadata(entity: BaseEntity): Promise<DuplicateMatch[]> {
        ...
        if (entity.type === 'PolicyDocument') {
            ...
            const entityJurisdiction = policyDoc.jurisdiction;
            const entityDate = policyDoc.date;

            for (const candidate of candidates) {
                if (candidate.id === entity.id) continue;

                const candidatePolicyDoc = candidate as PolicyDocument;
                const candidateJurisdiction = candidatePolicyDoc.jurisdiction;
                const candidateDate = candidatePolicyDoc.date;
    ...
    private async findByMaximumWeightMatching(entity: BaseEntity): Promise<DuplicateMatch[]> {
        ...
        try {
            ...
            logger.warn({ error, entityId: entity.id }, 'Maximum weight matching failed for entity');
            return [];
        }
    }
    ...
    async findOptimalMatches(
        ...
        try {
            ...
            logger.warn({ error }, 'Batch maximum weight matching failed');
            // Fallback to individual matching
            const results = new Map<string, Array<{ entity: BaseEntity; similarity: number }>>();
            for (const entity of entities) {
                const matches = await this.findDuplicates(entity);
