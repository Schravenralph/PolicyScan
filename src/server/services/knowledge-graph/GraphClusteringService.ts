...
export class GraphClusteringService {
    ...
    invalidateCache(): void {
        ...
        logger.info('Graph clustering cache invalidated');
    }
    ...
    async createMetaGraph(options: {
        ...
        if (this.metaGraphCache.has(cacheKey)) {
            logger.debug({ cacheKey, pathDepth, minClusterSize }, 'Returning cached meta-graph');
            return this.metaGraphCache.get(cacheKey)!;
        }

        logger.info({ cacheKey, pathDepth, minClusterSize }, 'Computing meta-graph');
        ...
        const edges = this.processEdgeMap(edgeMap);
    ...
    /**
     * Convert edge map to array with validation
     */
    private processEdgeMap(edgeMap: Map<string, number>): MetaEdge[] {
        const edges: MetaEdge[] = [];
        for (const [key, weight] of edgeMap.entries()) {
            // Validate edge key format to prevent incorrect splits
    ...
    async getClusterSubgraph(cluster: ClusterNode, options: {
        ...
        const { maxNodes = 500, maxDepth = 3 } = options;

        // Start from the representative node or first node
        const startNode = cluster.representativeNode || cluster.children[0];
        ...
        // Use the graph's getSubgraph but limit to nodes within this cluster
        ...
        // Filter to only include nodes that are in this cluster
        const clusterUrlSet = new Set(cluster.children);
        const filteredNodes: { [url: string]: NavigationNode } = {};

        for (const [url, node] of Object.entries(subgraph.nodes)) {
            if (clusterUrlSet.has(url)) {
        ...
        return {
            ...
            metadata: {
                ...
                nodesReturned: Object.keys(filteredNodes).length
            }
        };
    }
    ...
    private gridLayout(
        ...
        const { width: _width, height: _height, nodeSpacing } = options;
    ...
    private forceDirectedLayout(
        ...
        const { width, height, nodeSpacing, iterations } = options;
        const positions = new Map<string, NodePosition>();
    ...
    private circularLayout(
        ...
        const { width, height, nodeSpacing: _nodeSpacing } = options;
        const positions = new Map<string, NodePosition>();
