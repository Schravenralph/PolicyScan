...
export class EntitySchemaValidator {
    ...
    private spatialUnitSchema = this.baseEntitySchema.extend({
        ...
        geometry: z.unknown().optional(), // GeoJSON validation can be added later
    });
    ...
    validate(entity: BaseEntity): ValidationResult {
        ...
        try {
            ...
            if (error instanceof z.ZodError) {
                const issues: z.ZodIssue[] = ('issues' in error ? error.issues : (error as { errors?: z.ZodIssue[] }).errors || []) as z.ZodIssue[];
                errors.push(
